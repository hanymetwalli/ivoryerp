      ...
      >
        <div className="space-y-4" dir="rtl">
          ...
          <div>
            ...
            <Input
            type="file"
            // نقبل كل الامتدادات، لكن الباك إند سيرفض الـ Excel برسالة واضحة الآن
            accept=".csv, .xlsx, .xls"
            onChange={async (e) => {
                const file = e.target.files?.[0];
                if (file) {
                    ...
                    reader.onload = async () => {
                      try {
                        ...
                        if (response?.data?.success) {
                          ...
                          toast.success(`تم بنجاح: ${response.data.report.created} جديد`);
                          await loadData();
                        } else {
                          const errorMsg = response?.data?.error || "تعذر المعالجة";
                          const detail = response?.data?.details || "";
                          toast.error(errorMsg);
                          setImportReport({ 
                            total: 0, processed: 0, created: 0, updated: 0, skipped: 0, 
                            errors: [errorMsg + (detail ? ": " + detail : "")] 
                          });
                        }
                      } catch (err) {
