...
export default function ApprovalTimeline({ approvalChain = [], workflowId = null }) {
    const [internalChain, setInternalChain] = useState([]);
    ...
    const steps = workflowId ? internalChain : (approvalChain || []);

    if (loading) {
        return (
            <div className="flex items-center justify-center p-8 text-slate-500">
                <Clock className="w-5 h-5 animate-spin ml-2" />
                جاري تحميل مسار الاعتماد...
            </div>
        );
    }
